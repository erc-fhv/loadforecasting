...
class ModelTrainer:
    ...
    def __init__(self, test_config, use_multiprocessing = True):        
        ...
        self.preprocess_data(test_config)
    ...
    def preprocess_data(self):
        ...
        if self.test_config['pretraining_mode'] == 'store_and_load_weights' \
            ...
            modelAdapter = ModelAdapter.ModelAdapter(public_holidays_timestamps, train_size = len(all_standard_loadprofiles), dev_size = 0, 
                                                    add_tda_features=False, addLaggedPower=True, shuffle_data=False,
                                                    use_persistent_entropy = False, seed=0)
            X_model_pretrain, Y_model_pretrain = modelAdapter.transformData(all_standard_loadprofiles, None)
            with open(self.pretraining_filename, 'wb') as file:
                pickle.dump((X_model_pretrain, Y_model_pretrain, modelAdapter), file)    
            
            _, _ = self.train(pretrain_now = True, finetune_now = False)
