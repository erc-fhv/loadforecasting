...
class Tirex2:
    ...
    def train_model_auto(
        ...
        Args:
            ...
            eval_stride: Passed as a fixed (untuned) constructor kwarg to every
                trial's model - see Tirex2.__init__'s eval_stride for what it does.
                Default 1 = no subsampling. Cuts per-trial wall-clock since
    ...
    def _forecast(self,
        ...
        unstrided x/y_true (see _build_timeseries) - stride only subsamples which
        of the resulting, already-correctly-contextualized windows are actually
        sent to the (expensive) model.forecast() call, so the context never has
        gaps even when stride > 1.
        """
